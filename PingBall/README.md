<img src="https://github.com/MAZHARMIK/Interview_DS_Algo/blob/master/icon.png" align="right" />
<a href="https://github.com/MAZHARMIK"><img style="position: absolute; top: 0; left: 0; border: 0;" src="https://camo.githubusercontent.com/82b228a3648bf44fc1163ef44c62fcc60081495e/68747470733a2f2f73332e616d617a6f6e6177732e636f6d2f6769746875622f726962626f6e732f666f726b6d655f6c6566745f7265645f6161303030302e706e67" alt="Fork me on GitHub" data-canonical-src="https://s3.amazonaws.com/github/ribbons/forkme_left_red_aa0000.png"></a>

# PingBall - A Simple Fun Game  
[![Build Status](https://travis-ci.org/joemccann/dillinger.svg?branch=master)](https://travis-ci.org/joemccann/dillinger)  
A simple game :video_game: that I developed during COVID-19 quarantine days.

Download full game with one click here : <a href="https://github.com/MAZHARMIK/Self-made-Projects/blob/master/PingBall/PingBall.zip">**DOWNLOAD**</a>:file_folder:

<h3>Features of the Game : :pencil:</h3>
<ul>
<li>UI in start for Player's name :running:</li>
<li>Enable and Disable sound :sound: :mute:</li>
<li>Beautiful background :sunrise_over_mountains:</li>
<li>Level based difficulty :triangular_flag_on_post:</li>
<li>Lives of player :heart:</li>
<li>Sound effects :loudspeaker:</li>
<li>Score track :slot_machine:</li>
</ul>

<h3>Few screenshots of the game to get the feel :camera: :</h3>

1) **FrontUI**
<img src="https://github.com/MAZHARMIK/Self-made-Projects/blob/master/PingBall/gameScreenshots/FrontUI.jpg">

2) **GameUI**
<img src="https://github.com/MAZHARMIK/Self-made-Projects/blob/master/PingBall/gameScreenshots/GameUI.jpg">

3) **GameOver**
<img src="https://github.com/MAZHARMIK/Self-made-Projects/blob/master/PingBall/gameScreenshots/GameOver.jpg">


<h2>Some small instructions to be followed before you enjoy  :video_game: :</h2>

:radio_button:**Sound of the Game to work in your browser** : 
If sound does't work in your browser, it is because you have not enabled the permission for Autoplay :disappointed_relieved:

&nbsp;&nbsp;&nbsp;&nbsp;To fix that :wrench::
  <ul>
    <li>Go to settings</li>
	  <li>Privacy & Security</li>
	  <li>Permissions :arrow_right: Autoplay</li>
	  <li>Enable it :smiley:</li>
	  <li>Refresh the game or relaunch the game to enjoy the :sound: as well</li>
  </ul>
  
<h2>How to download and launch the game :video_game: :</h2>
&nbsp;&nbsp;&nbsp;&nbsp; Steps are as follows :
  <ul>
    <li>Click on the download button above to download the full game at once</li>
	  <li>You will get a .zip file :file_folder:</li>
	  <li>Open it and look for pingBall.html. Click on it</li>
    <li>Let's play :running:</li>
  </ul>
